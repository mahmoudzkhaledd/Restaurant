...
exports.userValidatorMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if (user == null || user.banned || emailVer) {
            return res.status(403).json({ msg: user.banned ? "تم حظر حسابك الرجاء التواصل مع الدعم الفني" : emailVer ? "الرجاء تفعيل الحساب أولا حتى تتمكن من القيام بهذه العملية" : null });
        } else {
